...
* New features:
  * **Files and folders can be excluded from the scan**: add `--exclude_paths` CLI argument and `exclude_paths` config key (under `[FILES]`), plus the `exclude_paths` attribute for library use. Accepts a comma-separated list of files and folders; an excluded file is never read and an excluded folder is skipped together with everything below it. Entries are literal paths (glob patterns such as `**/node_modules` are not supported) and may be absolute or relative to the directory salted was called from — *not* relative to the searchpath. A file named directly with `-i` is skipped as well if it is on the list. Backslashes are part of the path, so Windows paths can be written as usual; a path containing a comma can be wrapped in double quotes. An entry matching nothing is logged as a warning, so a typo does not silently check what was meant to be skipped. Unlike the other path settings, exclusions from an auto-discovered config file are not confined to that file's own folder: they can only ever reduce what salted reads.
...
* Security:
  ...
  * Control characters are now stripped from everything the report shows (link text, URLs, file paths). A checked document could otherwise inject terminal escape sequences and forge the result — printing a green "All links OK" inside the list of broken links, or overwriting a line with a carriage return.
  * The stripping of control characters now covers the C1 range (U+0080–U+009F) as well, not just C0 and DEL. C1 holds single-character equivalents of the escape introducers the C0 filtering was written for: U+009B is CSI, U+009D is OSC, and U+009C is the string terminator that closes an OSC 8 hyperlink. Terminals decoding UTF-8 mostly ignore them, but that is a property of the terminal rather than of the document being checked, and a console reading an 8-bit codepage does act on them.
  * Markdown reports now escape values taken from documents. An unescaped `|` ended a table cell and let a document inject columns or rows into the results table; `<`, `[` and `` ` `` carried markup into anything that renders the report. Link targets percent-encode the characters that end a markdown link, so URLs containing parentheses (e.g. Wikipedia's `..._(programming_language)`) no longer render as broken links.
...
* Bug Fixes:
  * The install commands for the optional extras are now quoted (`pip install "salted[bibtex]"`) in the documentation *and* in the message salted prints when pybtex is missing. Unquoted, zsh — the default shell on macOS — reads the brackets as a filename pattern and aborts with `no matches found` before pip runs, so the suggested fix failed for the users most likely to need it.
  * `--raise_for_dead_links` now also fails a run in which a file could not be read at all — missing, no permission, over `--max_file_size_mb`, or unparseable (such as a malformed `.bib`). Previously only the missing-pybtex case did this, while every other unreadable file was listed in the report's FILE ACCESS ERRORS section and the run still exited 0 — so a permission error or an oversized file silently stopped checking those links in exactly the pipeline meant to catch that. The exception message now names each reason the run failed instead of always saying "Found dead URLs".
  * Fix salted ending a run with a `UnicodeEncodeError` traceback whenever the report contained a character the console encoding cannot represent — a CJK, Cyrillic or emoji link text is enough. Attached to a terminal Python encodes stdout as UTF-8 (PEP 528), but a *redirected* stdout falls back to the locale encoding, which on Windows is the ANSI codepage (e.g. cp1252). Interactive runs were therefore fine while `salted -i . > report.txt` and every CI runner capturing stdout crashed — exactly the pipeline use case, and with exit code 1, so it looked like a dead-link failure. Characters the console cannot show are now replaced by `?` and a warning names the encoding; writing the report to a file (`--write_to`) still keeps them, as files are always written as UTF-8.
  * Fix a custom template being silently ignored when it was named like one of the built-ins. The decision to render the packaged template was made on the file name alone, so `--template_searchpath ./my-templates --template_name default.cli.jinja` rendered salted's own template and said nothing about it — a trap for anyone who started from a copy of the shipped template and kept its name. A custom search path now wins whenever it actually holds a file of that name; if it does not, the built-in still applies, so setting a search path without overriding the name keeps working as before.
