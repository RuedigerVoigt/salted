...
| [lxml](https://lxml.de/) | **Optional.** High-performance HTML parsing backend for BeautifulSoup — faster and more lenient with malformed HTML than the built-in `html.parser`. Install with `pip install salted[lxml]`. Falls back to `html.parser` if not available. Requires `>=6.0.2` (needed for Python 3.14 compatibility). |
